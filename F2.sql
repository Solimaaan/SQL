...
where id=@id)
begin
set @redirect='NonGucian'
end
if exists( select id
from Supervisor
where id=@id)
begin
set @redirect='Supervisor'
end
if exists( select id
from Examiner
where id=@id)
begin
set @redirect='Examiner'
end
if exists( select id
from Admin
where id=@id)
...
Create proc findThesis
@title varchar(30)
as
select *
from Thesis
where title LIKE '%'+@title+'%'   -- check procedure
go;

CREATE Proc ViewStudentThesisSupervisor
@id int
As
Select s.name,t.title,gs.firstName
From Examiner e inner join ExaminerEvaluateDefense eed on e.id = eed.examinerId
inner join  Thesis t on eed.serialNo = t.serialNumber 
inner join GUCianStudentRegisterThesis sr on
t.serialNumber=sr.serial_no
inner join Supervisor s on s.id=sr.supid inner join GucianStudent gs on sr.sid=gs.id
where e.id = @id and eed.date <=Current_TIMESTAMP   
union
Select s.name,t.title,gs.firstName
From Examiner e inner join ExaminerEvaluateDefense eed on e.id = eed.examinerId 
inner join  Thesis t on eed.serialNo = t.serialNumber
inner join NonGUCianStudentRegisterThesis sr on
t.serialNumber=sr.serial_no
inner join Supervisor s on s.id=sr.supid inner join GucianStudent gs on sr.sid=gs.id
where e.id = @id AND eed.date <=Current_TIMESTAMP
go
...
begin
select T.*
from GUCianStudentRegisterThesis GR inner join GucianStudent G on GR.sid=G.id inner join Thesis T on T.serialNumber=GR.serial_no
where G.id=@studentid
end
else
begin
select T1.*
from NonGUCianStudentRegisterThesis NG inner join NonGucianStudent N on NG.sid=N.id inner join Thesis T1 on T1.serialNumber=NG.serial_no
where N.id=@studentid
end
go
